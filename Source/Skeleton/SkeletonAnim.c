			...
			case	ANIMEVENT_TYPE_PLAYSOUND:
					if (!gDisableAnimSounds)
					{
						switch(eventValue)
						{
							...
							case	0:
									if (gLevelNum != -1)
										PlayEffect_Parms3D(EFFECT_GNOMESTEP, &theNode->Coord, NORMAL_CHANNEL_RATE, 1.0);
									break;
							...
							case	1:
									if (gLevelNum != -1)
										PlayEffect_Parms3D(EFFECT_GNOMESTEP, &theNode->Coord, NORMAL_CHANNEL_RATE*2/3, 1.0);
									break;
							...
							case	2:
									PlayEffect_Parms3D(EFFECT_TICKSTEP, &theNode->Coord, NORMAL_CHANNEL_RATE - (MyRandomLong() & 0x1fff), 1.0);
									break;
							...
							case	4:
									PlayEffect_Parms3D(EFFECT_SERVO, &theNode->Coord, NORMAL_CHANNEL_RATE * 2/3, .5);
									break;

							case	5:
									PlayEffect_Parms3D(EFFECT_SERVO, &theNode->Coord, NORMAL_CHANNEL_RATE * 1/2, .5);
									break;

									/* ANT BITE */

							case	6:
									PlayEffect_Parms3D(EFFECT_ANTBITE, &theNode->Coord, NORMAL_CHANNEL_RATE, .5);
									break;
							...
							case	8:
									PlayEffect3D(EFFECT_OTTOFALL, &theNode->Coord);
									break;

									/* FOOTSTEP */

							case	9:
									PlayEffect_Parms3D(EFFECT_FOOTSTEP, &theNode->Coord, NORMAL_CHANNEL_RATE + (MyRandomLong() & 0x3fff), .5);
									break;
...
float	diffA,diffB;
float	k2Percent,k1Percent;
long	accMode1,accMode2,accMode3;
const float	one = 1.0f;

				/* GET ACCELERATION MODE */


	accMode1 = kf1->accelerationMode;				// get k1 accmode
	accMode2 = kf2->accelerationMode;				// get k2 accmode

	if ((accMode1 == ACCEL_MODE_EASEINOUT) || (accMode1 == ACCEL_MODE_EASEOUT))		// see if ease out of k1
	{
		if ((accMode2 == ACCEL_MODE_EASEINOUT) || (accMode2 == ACCEL_MODE_EASEIN))	// see if ease in to k2
			accMode3 = ACCEL_MODE_EASEINOUT;
		else
			accMode3 = ACCEL_MODE_EASEOUT;
	}
	else																			// linear out of k1
	{
		if ((accMode2 == ACCEL_MODE_EASEINOUT) || (accMode2 == ACCEL_MODE_EASEIN))	// see if ease in to k2
			accMode3 = ACCEL_MODE_EASEIN;
		else
			accMode3 = ACCEL_MODE_LINEAR;
	}
...
float	x,n;
	...
	{
		x = i;
	    x = x / (float)CURVE_SIZE; 		/* normalize to [0:1] */
		n = (x*x * (3.0f - 2.0f*x));
